...
class ModelSaleCustomer extends Model {
    ...
    public function editCustomer($customer_id, $data) {
        ...
        if (isset($data['address'])) {
            foreach ($data['address'] as $address) {
                $sql = "INSERT INTO " . DB_PREFIX . "address SET address_id = '" . (int) $address['address_id'] . "', customer_id = '" . (int) $customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', company_id = '" . $this->db->escape($address['company_id']) . "', tax_id = '" . $this->db->escape($address['tax_id']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int) $address['country_id'] . "', zone_id = '" . (int) $address['zone_id'] . "'";
                $sql.= ", payment_cad_cpf = '" . $address['payment_cad_cpf'] . "',payment_numero = '" .  $address['payment_numero'] . "',payment_complemento = '" .  $address['payment_complemento'] . "'";
                $sql.= ", payment_cad_area_code = '" . $address['payment_cad_area_code'] . "'";
    ...
    public function getAddress($address_id) {
        ...
        if ($address_query->num_rows) {
            ...
            foreach (array_keys($this->getOrderInstance()->_mail_chimp_columns) as $field) {
                $return_arr[$field] = $address_query->row[$field];
            }
