...
class ModelCatalogProduct extends Model {
    ...
    public function getProduct($product_id) {
        ...
        if ($query->num_rows) {
            return array(
                ...
                'product_on_phone' => $query->row['product_on_phone'],
            );
        } else {
            return false;
        }
    ...
    public function getProductSpecials($data = array()) {
        ...
        $sql.= " AND p.referenc_id IS NULL GROUP BY ps.product_id ";
        $sort_data = array(
            'pd.name',
            'p.model',
            'ps.price',
    ...
    public function getLatestProducts($limit) {
        ...
        if (!$product_data) {
            ...
            $sql.= " AND p.referenc_id IS NULL " . " ORDER BY p.date_added DESC LIMIT " . (int) $limit;
            ;
            $query = $this->db->query($sql);
            foreach ($query->rows as $result) {
                $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
    ...
    public function getPopularProducts($limit) {
        $product_data = array();
        $sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'";
        $sql.= " AND p.referenc_id IS NULL ORDER BY p.viewed, p.date_added DESC LIMIT " . (int) $limit;
        $query = $this->db->query($sql);
    ...
    public function getBestSellerProducts($limit) {
        ...
        if (!$product_data) {
            ...
            $sql.= " AND p.referenc_id IS NULL " . " GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int) $limit;
    ...
    public function getTotalProducts($data = array(), $check = false) {
        if ($this->customer->isLogged()) {
            $customer_group_id = $this->customer->getCustomerGroupId();
        } else {
            $customer_group_id = $this->config->get('config_customer_group_id');
